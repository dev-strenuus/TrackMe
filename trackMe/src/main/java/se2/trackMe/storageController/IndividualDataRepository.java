...
public interface IndividualDataRepository extends CrudRepository<IndividualData, Long> {
    ...
    // @Query("SELECT i FROM IndividualData AS i WHERE (i.individual.fiscalCode = ?1) AND (i.timestamp BETWEEN <= ?2)")
    List<IndividualData> findAllByIndividualAndTimestampBefore(Individual individual, Date date);
