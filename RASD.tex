	...
	\begin{legal}\bfseries
 	\item \underline{Introduction} 
		...
		\item \textit{Scope}\\
    		...
    		\item Description of the system \\\\
			{\normalfont
This software system is thought to be a Health Data Management, Run-friendly App, useful for a quite wide range of customers with different necessities. According to these, the TrackMe client base will be mainly represented by: \\-common users, such as people with high/low blood pressure or other heart diseases who desire to be monitored; \\-runners, who may want to check their health parameters while performing or participate to some running events; \\-third parties, whose interest might be to store the users' health data and take statistics on them, or organize runs which inviduals will be free to join.\\ The system will fulfill the needs of the users, providing ad hoc services for each customer segment. In particular, the system's main goal is to facilitate the user's life through an optimal experience. This will be made possible thanks to several features: a quick communication between the actors because of a request-response approach, an high information reliability granted by an efficient data retrieval from the users' wearable devices, and other properties that will be will be further described in later chapters.
			}\\
			\item Goals \\\\
			{\normalfont
...
\textit{Track4Run} is a service thought for the organization of running events. It permits to third parties to organize events and to users to enrol to them or simply to know the position of runners during a race.\\
...
\newpage
	\item \underline{Specific requirements}\\
		...
		\item \textit{External interface requirements}\\
			...
			\item Group request
				\begin{figure}[H]
  				%\includegraphics[width=\linewidth]{./images/seq-diagrams/.png}
				\end{figure}
			\item Join a run 
				\begin{figure}[H]
  				%\includegraphics[width=\linewidth]{./images/seq-diagrams/.png}
